...
export class RequestsComponent implements OnInit {
  requests: (Request & { userName?: string })[] = [];
  approvalRequests: (Request & { userName?: string })[] = [];
  userId: number;
  ...
  loadApprovalRequests(): void {
    this.requestService
      .getRequestsForEventsCreatedByUser(this.userId)
      .subscribe((requests: (Request & { userName?: string })[]) => {
        requests.forEach((request) => {
          this.userService
            .getUserById(request.applicantId)
            .subscribe((user) => {
              request.userName = (user as any).username;
            });
        });
        this.approvalRequests = requests;
      });
